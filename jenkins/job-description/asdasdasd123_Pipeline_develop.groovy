...
def job_config = [
        job: [
                name: "asdasdasd123_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asdasdasd123_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asdasdasd123_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asdasdasd123.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asdasdasd123", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asdasdasd123_Pipeline_develop");

job.with {

    authenticationToken('3kzXAYi3fllJjanQQ5c8FROgoso06nBw')
